...
// Shared schema factory — bound to two different connections (db1, the
// original cluster that filled up and now holds the "everything else"
// snapshot; db2, the dedicated cluster real estate/interior designer/
// construction now actively grow on). A factory (not a single schema
// instance) because Mongoose schemas can't be reused across models safely.
export function buildBniLeadSchema() {
  ...
  schema.index({ companyName: "text", displayName: "text", business: "text" });
  // Common combined filters (industry + contact-info toggles) at list-page scale.
  schema.index({ industryKeyword: 1, hasEmail: 1, hasPhone: 1 });
  schema.index({ isIndian: 1, industryKeyword: 1 });
